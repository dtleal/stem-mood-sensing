...
class UploadMoodCaptureRepository:
    ...
    async def upload_mood_capture(
        ...
        async with self._db.session() as session:
            try:
                user_id = await UserModel.find_by_name(session=session, user_name=input_port.user_name)

                if not user_id:
                    await self._insert_user(session=session, user_name=input_port.user_name)
                    user_id = await UserModel.find_by_name(session=session, user_name=input_port.user_name)

                await self._insert_mood(session=session, user_id=user_id, input_port=input_port)
            except Exception as error:
                raise(f'An error has ocurred: {error}')
